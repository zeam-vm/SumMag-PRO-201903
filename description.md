...
map-map フュージョンを行なった場合，`Enum.map`部分は複数のプログラム業に由来することになり，実行時環境に `[[line: 8], [line: 9]]` のように複数行を記録する必要が出てくる．このような実行時環境の与え方について，Erlang VM そのものは許容していることを確認できた．

しかし，ソースコードレベルデバッガなどを接続した場合に，複数行を含む実行時環境がどのように扱われるかは不分明である．このような複数行にまたがる AST レベルの最適化を踏まえた，ツール相互運用性について検討し，必要ならば整合するようにツールの仕様を修正する必要があると考えている．
