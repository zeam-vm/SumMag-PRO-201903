...
SumMag では，まず関数定義を読み取る．各関数の本体に，1個以上のパイプライン演算子と1個以上の`Enum.map`が連なったコード片がないか，マッチングする．もしマッチしなければ，その関数全体をそのままコード生成する．

マッチした場合について説明する．
`a |> Enum.map(...)` のように1つずつのパイプライン演算子と `Enum.map` があった場合と，`a |> Enum.map(...) |> Enum.map(...) ...` のように2個以上ずつのパイプライン演算子と `Enum.map` があった場合では，ASTの形状が異なる．前者だと，直接3つ組のノードを直接指し，後者だと，3つ組のリストとなる．またパイプラインが複数連なる場合は，ツリー状に構成される．すなわち，パイプライン演算子の3つ組の子ノードとして `Enum.map`の3つ組と次のパイプライン演算子の3つ組が入るという構成になる．

map-map フュージョンは次のように行う．
...
map-map フュージョンの後で，パイプライン演算子と`Enum.map`の組合わせ部分を切り出して，独立した関数を定義して格納する．元あったコードを，生成した関数の呼び出しのコードに置き換える．
