...
このような背景で我々は Hastega (ヘイスガ)のプロトタイプを開発した\cite{ZACKY18J, Hisae19J, ZACKY19-Hastega, ZACKY19-Hastega-Medium, ZEAM-log}．Hastega は，ウェブアプリケーションの中で行う画像処理や機械学習などの計算負荷の高い処理を，Elixirで簡潔に記述し，サーバー上やエッジ，ウェブクライアント上の CPU や GPU に負荷分散させながら高速に並列実行させることを目指している\cite{ZACKY19-Hastega, ZACKY19-Hastega-Medium}．Hastega の実現により，情報量量爆発の問題に対抗するための有効な計算能力を容易に向上できる．
...
Hastega は `defhastega` を読み取ると，いくつかの情報とともに，`do` ブロックの中身を SumMag に渡す．

SumMag では，まず関数定義を読み取る．各関数の本体に対し，次のように map-map フュージョンを試みる．

1. ASTを再帰的に走査して，パイプライン演算子のネストを検出する．
2. ネストが存在した場合は，第3要素がパイプライン演算子の3つ組と `Enum.map` の3つ組のリストであるかマッチを行い，マッチする場合はさらにそのパイプライン演算子の子ノードに `Enum.map` が存在するかマッチを行う．
3. これもマッチするならば，ASTを操作して2つの`Enum.map`を1つの`Enum.map`にまとめあげる．その引数には無名関数のASTをつなぐ．その無名関数の本体は，元の関数2つをパイプライン演算子でつなぎ合わせた関数を合成したものとする．
4. このような操作を再帰的に行い，連続する`Enum.map`をすべて1つの`Enum.map`にまとめあげるまで進める．

map-map フュージョンの後で，パイプライン演算子と`Enum.map`の組合わせ部分を切り出して，独立した関数を定義して格納する．元あったコードを，生成した関数の呼び出しのコードに置き換える．パイプライン演算子と`Enum.map`の組合わせ部分が存在しない場合は，そのまま出力する．

map-map フュージョンを行なった場合，`Enum.map`部分は複数のプログラム行に由来することになり，実行時環境に `[[line: 8], [line: 9]]` のように複数行を記録する必要が出てくる．このような実行時環境の与え方について，Erlang VM そのものは許容していることを確認できた．

しかし，ソースコードレベルデバッガなどを接続した場合に，複数行を含む実行時環境がどのように扱われるかは不分明である．ツール相互運用性に関して，本報告のような複数行にまたがる AST レベルの最適化についても整合するようにツールの仕様を修正する必要があると考えている．

# まとめと将来課題
\label{sec:summary}

本報告では，まず，Elixir が生み出された背景について述べ，Hastega を研究開発するに至った動機と原理を詳述した．次に，Hastega が最適化の対象とする `Enum.map` とパイプライン演算子で構成される一連のコード片を抽出するメタプログラミングライブラリである SumMag について詳述した．

Elixir は，リストとタプルからなる抽象構文木(AST)を操作するようなメタプログラミング機構，Elixir マクロを提供しているので，SumMag の実装に用いた．

SumMag は，`Enum.map` とパイプライン演算子で構成される一連のコード片がどのような形状をしているかを踏まえて，まず再帰的に map-map フュージョンを行い，その後で，その部分を切り出して，独立した関数を定義して格納し，元あったコードを，その関数への呼び出しコードに置き換える．

近い将来課題としては，Magicite を含む Hastega の残りの機能を実装してファーストリリースとして完成させ
OSS として公開する．現在，2019年夏までのリリースを目指している．
